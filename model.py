...
from visbackbone.swin import EncImgSwinMean, EncImgSwinConcat
from visbackbone.merlot import EncImgMerlot
from visbackbone.resnet50 import EncImgR50Concat, EncImgR50Mean
...
class VIOLET_Base(T.nn.Module):
    def __init__(self, args, tokzr=None):
        ...
        else: self.trsfr = bert.bert.encoder
    ...
    def go_cross(self, feat_img, mask_img, feat_txt, mask_txt, attn_mask_type="full", feat_pretxt=None, mask_pretxt=None):
        if feat_pretxt is not None:
            ...
            feat = T.cat([feat_img, feat_pretxt, feat_txt], dim=1)
        else: feat = T.cat([feat_img, feat_txt], dim=1)
